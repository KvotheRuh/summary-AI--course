    ...
    return (
      <div className="quiz-result">
        ...
        <div className="quiz-result__actions">
          <button className="quiz-result__btn-back" onClick={() => onBack(null)}>
            Voltar
          </button>
          <button className="quiz-result__btn-retry" onClick={onMoreQuestions}>
            {mode === "topico" ? "Próximo tópico →" : "Mais 15 questões →"}
          </button>
        </div>
      </div>
    );
    ...
    <div className="quiz-engine">
      <div className="quiz-engine__body">
        <div className="quiz-engine__progress">
          <button className="quiz-engine__back-btn" onClick={() => onBack(null)}>←</button>
        ...
        {question.image && (
          <div className="quiz-engine__question-img">
            <img src={question.image} alt="Imagem da questão" />
          </div>
        )}
        ...
        <div className="quiz-engine__options">
          {question.options.map((opt, idx) => (
            <button
              key={idx}
              className={`quiz-engine__option ${
                selected === null
                  ? ""
                  : idx === question.correct
                  ? "quiz-engine__option--correct"
                  : idx === selected
                  ? "quiz-engine__option--wrong"
                  : ""
              }`}
        ...
        {/* Explicação + imagem da explicação */}
        {selected !== null && (
          <div className="quiz-engine__explanation">
            ...
            {question.explanationImage && (
              <img
                ...
                alt="Explicação visual"
                className="quiz-engine__explanation-img"
              />
            )}
          </div>
        )}
